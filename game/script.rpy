...
define rs = Character(_("Suitemates"), color="#ff503d")
...
label end_of_year_no_housing:
    scene sad_background
    with fade
    "You've waited until the end of the year to find housing. Unfortunately, you could not secure a spot anywhere. Your only only options are to either couch surf or live hotel."
    menu:
        "Couch surf":
            jump couch_surf
        "Live in Hotel":
...
label couch_surf:
    if roommate <= 1:
        scene black_background
        with fade
        "It seems like you have not made enough friends throughout the year. So, couch surfing is out of the question. The only remaining option is to live in a hotel."
        jump live_in_hotel
    else:
        "Let make sure with your roommates if they are okay with you couch surf once in a while."
        scene roommates
        with fade
        "Oh look, it's your last year's roommates."
        rs "Hello there. How have you been? We have not seen you in a while :)"
        m "Oh hey! Ummmm. Im supposedly doing okay. Just trying to find housing for next year but I have become a victim of the housing crisis."
        rs "Oh no! Many of our friends are in the same situation. Hopefully, the university does something about it. Let us know if there is anything we can do to help you."
        m "Actually yes. I was gonna ask if you guys were okay if I could couch surf once in a while in the worst case scenario."
        rs "Of course, buddy. That is what friends are for :)"

        scene black_background
        "You start couch surfing. Weeks pass by."
        "..."
        "Yours friends are getting little uncomfortable. You are getting lot of stress because you still cant find any housing."
        $ stress +=  5
        jump end_stress_check
...
label live_in_hotel:
    if money > 10000 :
        ...
        $ money = 0
    scene hotel
    "Several weeks pass by living in the hotel."
    "..."
    "You still cant find housing and you are going out of pocket."
    $ stress += 5

label end_stress_check:
    if stress > max_stress or money == 0 :
        scene sad_background
        "Unfortunately, college life has been very rough on you. You are on the verge of getting homeless which is slowly leading you into the depression. You should probably dropout."
    jump end_of_all_states
